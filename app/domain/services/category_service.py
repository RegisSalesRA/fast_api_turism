...
class CategoryService: 
    ...
    def create_point(self, entity: CategoryEntity) -> CategoryEntity:
        if not entity.name or entity.name.strip() == "":
            raise ValueError("name must not be empty")
        created = self.repo.create(entity)
        return created

    def list_points(self, *, limit: Optional[int] = None, offset: Optional[int] = None) -> List[CategoryEntity]:
        return self.repo.list_all(limit=limit, offset=offset)

    def get_point(self, id: int) -> CategoryEntity:
        p = self.repo.get_by_id(id)
        if p is None:
            raise NotFoundError(f"Point with id {id} not found")
        return p

    def update_point(self, entity: CategoryEntity) -> CategoryEntity:
